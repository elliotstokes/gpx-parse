...
var _getWayPoints = function(gpxWaypoints) {
	...
	if (gpxWaypoints && gpxWaypoints.length) {
      ...
      var point = new GpxWaypoint(wayPoint.$.lat, wayPoint.$.lon, getElevation(wayPoint.ele), wayPoint.time);
...
/**
 * Gets the elevation from the item
 **/
var getElevation = function(item) {
  var elevation = null;
  if (item && Array.isArray(item) && item.length > 0) {
    elevation = parseFloat(item[0]);
  }
  return elevation;
};
    ...
    gpxTracks.forEach(function(currentTrack) {
      ...
      currentTrack.trkseg.forEach(function(currentSegment) {
        ...
        currentSegment.trkpt.forEach(function(trackPoint) {
  				var elevation = getElevation(trackPoint.ele);
